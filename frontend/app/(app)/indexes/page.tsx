...
import { Plus, X, Search, RotateCcw, Trash2, Layers } from 'lucide-react';
...
interface SavedIndex { id: string; name: string; holdingCount: number; }
interface QuoteData {
...
function AssetSearch({ onAdd, api, existing }: { onAdd: (s: string) => void; api: string; existing: string[] }) {
  ...
  return (
    <div ref={ref} className="relative">
      <div onClick={() => setOpen(true)} className="flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-950 hover:border-indigo-400 cursor-pointer transition-colors">
        ...
        <div className="absolute top-11 left-0 z-50 w-72 bg-white dark:bg-slate-900 rounded-xl shadow-xl dark:shadow-none border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="p-3 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 focus-within:border-indigo-400 transition-all">
              ...
              <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search ticker or company…"
          ...
          {results.length > 0 ? (
            ...
            <p className="text-xs text-slate-400 px-3 py-4 text-center">No results for &ldquo;{query}&rdquo;</p>
          ) : (
            <p className="text-xs text-slate-400 px-3 py-4 text-center">Type to search…</p>
          )}
        </div>
      )}
    </div>
...
export default function IndexesPage() {
  ...
  const [savedList, setSavedList]     = useState<SavedIndex[]>([]);
  ...
  function loadList() {
    if (!user) { setSavedList([]); setListLoading(false); return; }
    setListLoading(true);
    supabase
      .from('custom_indexes')
      .select('id, name, index_holdings(asset_symbol)')
      .eq('user_id', user.id)
      .order('updated_at', { ascending: false })
      .then(({ data }) => {
        setSavedList((data ?? []).map((d: any) => ({
          id: d.id, name: d.name, holdingCount: (d.index_holdings ?? []).length,
        })));
        setListLoading(false);
      });
  }

  useEffect(() => { loadList(); }, [user]); // eslint-disable-line
  ...
  const equalSplit = (list: Holding[]): Holding[] => {
    const n = list.length, base = Math.floor(100 / n), extra = 100 - base * n;
  ...
  const newIndex = () => { setCurrentId(null); setIndexName(''); setHoldings([]); setSaveError(''); };
  ...
  const saveIndex = async () => {
    ...
    if (!indexId) { setSaveError('Failed to create index.'); setSaving(false); return; }
  ...
  const ws = weightOk
    ? { label: `${totalWeight}% ✓`, cls: 'text-emerald-600 dark:text-emerald-400' }
    : totalWeight > 100
      ? { label: `${totalWeight}% ↑ over`, cls: 'text-rose-600 dark:text-rose-400' }
      : { label: `${totalWeight}%`, cls: 'text-amber-600 dark:text-amber-400' };
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-100">Indexes</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Build and manage your custom weighted indexes</p>
      </div>

      <div className="flex gap-5 items-start">

        {/* ── Saved list panel ── */}
        <div className="w-56 shrink-0 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between mb-0.5">
              <div className="flex items-center gap-2">
                <Layers size={13} className="text-indigo-500 shrink-0" />
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">My Indexes</span>
              </div>
              {savedList.length > 0 && (
                <span className="text-[10px] font-medium text-slate-400 dark:text-slate-500 tabular-nums">{savedList.length}</span>
              )}
            </div>
          </div>
          ...
          {listLoading ? (
            ...
            <div className="px-4 py-8 flex flex-col items-center gap-2 text-center">
              <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center">
                <Layers size={14} className="text-slate-300 dark:text-slate-600" />
              </div>
              <p className="text-xs text-slate-400 dark:text-slate-600">No indexes yet</p>
            </div>
          ) : (
            <ul>
              {savedList.map((item) => (
                <li key={item.id}
                  ...
                  onClick={() => loadIndex(item.id)}
                >
                  <div className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors ${currentId === item.id ? 'bg-indigo-500' : 'bg-slate-200 dark:bg-slate-700'}`} />
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-semibold truncate ${currentId === item.id ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-200'}`}>
                      {item.name}
                    </p>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{item.holdingCount} holdings</p>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteIndex(item.id); }}
                    disabled={deletingId === item.id}
                    className="shrink-0 p-1 rounded text-slate-300 dark:text-slate-600 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all disabled:opacity-50"
                  >
                    <Trash2 size={11} />
                  </button>
                </li>
          ...
          <div className="px-3 py-2.5 border-t border-slate-100 dark:border-slate-800">
            <button onClick={newIndex}
              className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950 transition-colors">
        ...
        <div className="flex-1 space-y-5 min-w-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none p-5 space-y-4">
              ...
              {holdings.length === 0 ? (
                /* Empty state — integrated into card */
                <div className="space-y-4">
                  <div className="py-4 flex flex-col items-center text-center gap-1">
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-400">Start from a template</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">or add assets below to build from scratch</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {PRESETS.map((p) => (
                      <button key={p.label}
                        onClick={() => { setHoldings(p.holdings); setCurrentId(null); setIndexName(''); }}
                        className="px-3 py-1.5 rounded-full text-xs font-medium bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950 transition-colors">
                        {p.label}
                      </button>
                    ))}
                  </div>
                  <AssetSearch onAdd={addHolding} api={api} existing={[]} />
                </div>
              ) : (
                <>
                  ...
                  {user && (
                    <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                      <input value={indexName} onChange={(e) => setIndexName(e.target.value)}
                        placeholder="Name this index…"
                        className="w-full text-sm px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-indigo-400" />
                      <div className="flex items-center justify-between">
                        <button onClick={newIndex}
                        ...
                        <button onClick={saveIndex} disabled={saving || !indexName.trim() || !holdings.length}
                          className={`text-xs px-4 py-1.5 rounded-lg font-medium transition-colors ${saved ? 'bg-emerald-600 text-white' : 'bg-indigo-600 hover:bg-indigo-700 text-white disabled:opacity-50'}`}>
                          {saved ? 'Saved ✓' : saving ? 'Saving…' : currentId ? 'Update' : 'Save'}
                        </button>
                      </div>
                      {saveError && <p className="text-xs text-rose-600 bg-rose-50 dark:bg-rose-950 border border-rose-100 dark:border-rose-900 rounded-lg px-3 py-2">{saveError}</p>}
                    </div>
                  )}
                </>
              )}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none p-5 flex flex-col">
              <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-4">Allocation</h2>
              {donutData.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center flex-1 py-8 text-center gap-3">
                  <div className="w-24 h-24 rounded-full border-[3px] border-dashed border-slate-200 dark:border-slate-700" />
                  <p className="text-xs text-slate-400 dark:text-slate-500">Add assets to see allocation</p>
                </div>
              )}
            </div>
          </div>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              {q?.name && <p className="text-xs text-slate-400 truncate max-w-[160px]">{q.name}</p>}
                            </div>
                          </div>
                        </td>
