...
import { Eye, Pencil, Trash2, TrendingUp, Plus } from 'lucide-react';

interface IndexHolding { symbol: string; weight: number; }
...
export default function LibraryIndexPage() {
  ...
  const [items, setItems] = useState<LibraryIndex[]>([]);
  ...
  function loadItems() {
    ...
    supabase
      ...
      .then(({ data }) => {
        setItems((data ?? []).map((idx: any) => ({
          id: idx.id,
          name: idx.name,
          updatedAt: idx.updated_at,
          holdings: (idx.index_holdings as any[])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-100">Custom Indexes</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Saved indexes from the Index Builder</p>
        </div>
        <button
      ...
      {!loading && items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400 dark:text-slate-600">
          <TrendingUp size={44} strokeWidth={1.25} className="opacity-25" />
          <p className="text-sm">No saved indexes yet</p>
          <button onClick={() => router.push('/index-builder')} className="text-xs text-indigo-500 hover:text-indigo-600 font-medium">
            ...
            return (
              <div key={item.id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  <div className="flex flex-wrap gap-1">
                    {item.holdings.length === 0 ? (
                      ...
                      const pct = totalWeight > 0 ? ((h.weight / totalWeight) * 100).toFixed(1) : null;
                      return (
                        <span
                          key={h.symbol}
                ...
                <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => router.push(`/index-builder?id=${item.id}`)}
                    title="View"
