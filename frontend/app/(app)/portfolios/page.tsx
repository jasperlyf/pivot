...
import { Search, X, RotateCcw, Plus, Trash2, Briefcase } from 'lucide-react';
...
interface SavedPortfolio { id: string; name: string; holdingCount: number; }
interface QuoteData {
...
export default function PortfoliosPage() {
  ...
  function loadList() {
    ...
    supabase
      .from('portfolios')
      .select('id, name, portfolio_assets(asset_symbol)')
      .eq('user_id', user.id)
      .order('updated_at', { ascending: false })
      .then(({ data }) => {
        setSavedList((data ?? []).map((d: any) => ({
          id: d.id, name: d.name, holdingCount: (d.portfolio_assets ?? []).length,
        })));
  ...
  const newPortfolio = () => { setCurrentId(null); setPortfolioName(''); setHoldings([]); setSaveError(''); };
  ...
  const savePortfolio = async () => {
    ...
    if (!portfolioId) { setSaveError('Failed to create portfolio.'); setSaving(false); return; }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Build and manage your portfolio strategies</p>
      </div>
          ...
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">My Portfolios</span>
              </div>
              {savedList.length > 0 && (
          ...
          {listLoading ? (
            ...
            <div className="px-4 py-8 flex flex-col items-center gap-2 text-center">
              <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center">
                <Briefcase size={14} className="text-slate-300 dark:text-slate-600" />
              </div>
              <p className="text-xs text-slate-400 dark:text-slate-600">No portfolios yet</p>
            </div>
          ) : (
            <ul>
              {savedList.map((item) => (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none p-5 space-y-4">
              ...
              {holdings.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PRESETS.map((preset) => (
                      <button key={preset.label}
                        onClick={() => { setHoldings(preset.holdings); setCurrentId(null); setPortfolioName(''); }}
                  ...
                  <AssetSearch api={api} onSelect={addHolding} />
                </div>
              ) : (
                <>
