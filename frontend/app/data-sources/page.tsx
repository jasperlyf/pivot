  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Upload CSV or Excel files to analyze</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
        <input
          ...
          className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 placeholder:text-slate-400"
        />
        <div
          onDrop={handleDrop}
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-1">CSV or Excel (.xlsx) — date, asset_name, value, category columns</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="text-sm font-semibold text-slate-900">Your Datasets</h2>
        </div>
        {datasets.length === 0 ? (
          <div className="px-5 py-10 text-center text-sm text-slate-400">No datasets yet</div>
        ) : (
          <ul>
            {datasets.map((d, i) => (
              <li key={d.id} className={`flex items-center justify-between px-5 py-3.5 ${i < datasets.length - 1 ? 'border-b border-slate-100' : ''}`}>
